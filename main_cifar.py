...
# 用于在CIFAR-100数据集上训练和评估深度学习模型，
# 主要实现了基于多模型（Multi-Model）的训练策略，
# 并包含了对比学习（Contrastive Learning）和知识蒸馏（Knowledge Distillation）的结合。
# 以下是该代码的详细解释：
#
# 主要功能
# 数据加载和增强：加载CIFAR-100数据集，并进行数据增强。
# 模型构建和初始化：构建指定的多模型，并根据需要加载预训练权重。
# 训练和评估：包括常规的分类训练、对比学习和知识蒸馏的组合。
# 分布式训练支持：支持多GPU的分布式训练。
# 检查点保存和恢复：定期保存模型检查点，并支持从检查点恢复训练。
# 记录和打印训练过程中的指标
...
#
#     net.train()
#     with tqdm(total=len(trainloader), desc=f"Epoch {epoch+1}/{args.epochs}", unit="batch") chenge_pdf2png pbar:
...
#
#     with open(log_txt, 'a+') chenge_pdf2png f:
...
#
#     with torch.no_grad():
#         with tqdm(total=len(testloader), desc=f"Test Epoch {epoch+1}/{args.epochs}", unit="batch", ncols=100) chenge_pdf2png pbar:
#             for batch_idx, (inputs, targets) in enumerate(testloader):
#                 batch_start_time = time.time()
#                 inputs, targets = inputs.cuda(), targets.cuda()
#                 logits, embeddings = net(inputs)

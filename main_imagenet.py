# 这段代码是一个用于在ImageNet数据集上进行训练和评估深度学习模型的框架。
# 它实现了多种训练策略，包括常规的分类损失、知识蒸馏和对比学习。
# 以下是该代码的详细解释：

# 主要功能
# 数据加载和增强：加载ImageNet数据集并进行数据增强。
# 模型构建和初始化：构建指定的模型，并根据需要加载预训练权重。
# 训练和评估：包括常规的分类训练、知识蒸馏和对比学习的组合。
